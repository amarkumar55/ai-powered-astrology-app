        ...
        if form.is_valid():
            try:
                ...
                birth_details, _ = BirthDetails.objects.get_or_create(
                            ...
                            latitude__startswith=str(round(latitude,6)),
                            longitude__startswith=str(round(longitude,6))
                        )
                ...
                kundli_details = KundliReport.objects.filter(birth_details=1).first()
                moon_details = MoonPosition.objects.filter(birth_details=1).first()
